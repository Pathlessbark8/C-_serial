...
void outer_dGx_pos(double G[], int i)
{
...
void outer_dGx_neg(double G[], int i)
{
    int j, k, r;
    double rho, u1, u2, pr;
    double x_i, y_i, x_k, y_k;
...
__device__ void outer_dGx_pos_cuda(points &point, double G[], int i, double VL_CONST, double gamma_new, int power)
{
    ...
    for (j = 1; j <= point.xpos_nbhs[i]; j++)
    {
        ...
        flux_quad_GxIII_cuda(G_i, nx, ny, u1, u2, rho, pr);

        qtilde_to_primitive_cuda(qtilde_k, u1, u2, rho, pr, gamma_new);
        flux_quad_GxIII_cuda(G_k, nx, ny, u1, u2, rho, pr);
        for (int r = 0; r < 4; r++)
        {
            sum_delx_delf[r] = sum_delx_delf[r] + (G_k[r] - G_i[r]) * dels_weights;
            sum_dely_delf[r] = sum_dely_delf[r] + (G_k[r] - G_i[r]) * deln_weights;
...
__device__ void outer_dGx_neg_cuda(points &point, double G[], int i, double VL_CONST, double gamma_new, int power)
{
...

    {
        ...
        qtilde_to_primitive_cuda(qtilde_i, u1, u2, rho, pr, gamma_new);
        flux_quad_GxIV_cuda(G_i, nx, ny, u1, u2, rho, pr);

        qtilde_to_primitive_cuda(qtilde_k, u1, u2, rho, pr, gamma_new);
        flux_quad_GxIV_cuda(G_k, nx, ny, u1, u2, rho, pr);
