...
void read_input_point_data()
{
    ...
    int i, k, r, nproc;
    int wall_temp, outer_temp, interior_temp, shape_temp;
    // character(len=10) :: itos

    std::fstream fin;
    fin.open("partGrid", std::ios::in);

    fin >> nproc >> max_points >> local_points >> ghost_points;
    allocate();
    ...
    if (format == 1)
    {
        ...
        for (k = 1; k <= local_points; k++)
        {
            fin >> point.original_id[k] >> point.x[k] >> point.y[k] >> point.left[k] >> point.right[k] >> point.flag_1[k] >> point.flag_2[k] >> point.min_dist[k] >> point.nbhs[k] >> (point.conn[k][r], r = 1, point.nbhs[k]);
            ...
            else if (point.flag_1[k] == 1)
            {
                interior_points = interior_points + 1;
            }
            else if (point.flag_1[k] == 2)
            {
    ...
    { // quadtree format
        for (k = 1; k <= local_points; k++)
        {
            // fin>>point.original_id[k]>> point.x[k]>> point.y[k]>>
            ...
            if (point.flag_2[k] > 0)
            {
                if (point.flag_2[k] > shapes)
                {
                    printf("shapes value wrong, check again");
                    exit(1);
                }
    ...
    wall_points_index = std::vector<int>(wall_points);
    interior_points_index = std::vector<int>(wall_points);
    outer_points_index = std::vector<int>(wall_points);
    shape_points_index = std::vector<int>(wall_points);
    ...
    if (proc > 1)
        allocate(pghost(ghost_points))
            ...
            do k = 1,
               ghost_points
                   read(101, *) pghost[k],
               &&point.x(local_points + k), point.y(local_points + k), &&point.min_dist(local_points + k)
