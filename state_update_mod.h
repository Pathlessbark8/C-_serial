...
__device__ void primitive_to_conserved_cuda(double prim[], double nx, double ny, double U[])
{
...
__device__ void conserved_vector_Ubar_cuda(double prim[], double Ubar[], double nx, double ny, double mach, double theta)
...

{
    ...
    if (point.flag_1[i] == 0)
    {
        ...
        primitive_to_conserved_cuda(temp1, nx, ny, U);
        primitive_to_conserved_cuda(temp2, nx, ny, U_old);
    ...
    else
    {
        ...
        conserved_vector_Ubar_cuda(temp1, U, nx, ny, mach, theta);
        conserved_vector_Ubar_cuda(temp2, U_old, nx, ny, mach, theta);
...
__device__ void conserved_to_primitive_cuda(double U[], double prim[])
{
...
__global__ void func_delta_cuda(points &point, double cfl)
{
    int bx = blockIdx.x;
    int tx = threadIdx.x;
    int i, k;
